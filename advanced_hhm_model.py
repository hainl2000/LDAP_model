...
from advanced_ldagm import EnhancedLDAGM
...
class JointVGAE_LDAGM(nn.Module):
    ...
    def __init__(self, num_lnc, num_diseases, num_mi, vgae_in_dim, vgae_hidden_dim, vgae_embed_dim, 
        ...
        # Enhanced LDAGM component for link prediction
        ...
        self.ldagm = EnhancedLDAGM(
            ...
            use_residual=config.USE_RESIDUAL_CONNECTIONS,  # Enable residual connections
            use_attention_aggregate=config.USE_ATTENTION_AGGREGATION,  # Enable attention-based aggregation
            num_attention_heads=config.ATTENTION_HEADS  # Number of attention heads
        )
...
def focal_loss(predictions, targets, alpha=0.25, gamma=2.0, reduction='mean'):
    ...
    # Compute focal loss
    focal_loss_val = focal_weight * bce_loss
...
def joint_loss_function(reconstructed_adj, original_adj, mu, log_var, 
    ...
    """
    Unified loss function for joint end-to-end training with focal loss support.
    ...
    Args:
        ...
        use_focal_loss: Whether to use focal loss for link prediction
    ...
    # Link prediction loss - use focal loss if specified
    if use_focal_loss:
        link_prediction_loss = focal_loss(
...
def joint_train(num_lnc, num_diseases, num_mi, train_dataset, multi_view_data, 
               ...
               use_focal_loss=config.USE_FOCAL_LOSS, focal_alpha=config.FOCAL_ALPHA, focal_gamma=config.FOCAL_GAMMA):
    """
    ...
    dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=24, pin_memory=True)
