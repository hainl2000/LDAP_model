"""
Hyperparameter Tuning Script for Joint End-to-End Training using Optuna
This script uses Optuna for efficient hyperparameter optimization and trains/tests
exactly like joint_end_to_end_main.py
"""
...
# Optuna will intelligently sample from these ranges
...
def train_and_evaluate_single_fold(
    fold, dataset, num_lnc, num_diseases, num_mi,
    train_dataset, test_dataset, multi_view_data,
    lnc_di_tensor, lnc_mi_tensor, mi_di_tensor,
    hyperparams, device
):
    """
    Train and evaluate a single fold with given hyperparameters.
    This function replicates the exact training and testing logic from joint_end_to_end_main.py
    ...
    Args:
        ...
        multi_view_data: Multi-view adjacency matrices
        lnc_di_tensor: lncRNA-disease interaction tensor
        lnc_mi_tensor: lncRNA-miRNA interaction tensor
        mi_di_tensor: miRNA-disease interaction tensor
...
def load_fold_data(dataset, device, fold):
    """Load data for a specific fold (to avoid reloading in objective)"""
    import numpy as np
    import pandas as pd
    import copy
    import torch
    from joint_end_to_end_main import JointDataset
...
def objective(trial, dataset, device, total_folds=5):
    ...
    Args:
        ...
        dataset: Dataset name
        device: Training device
        total_folds: Number of cross-validation folds
        
    Returns:
        Average AUC across all folds (optimization target)
    """
    ...
    for fold in range(total_folds):
        ...
        fold_metrics = train_and_evaluate_single_fold(
            ...
            lnc_di_tensor=fold_data["lnc_di_tensor"],
            lnc_mi_tensor=fold_data["lnc_mi_tensor"],
            mi_di_tensor=fold_data["mi_di_tensor"],
...
def hyperparameter_search(n_trials=N_TRIALS, study_name=None):
    ...
    Args:
        ...
        study_name: Optional name for the study (for resuming)
        
    Returns:
        Optuna study object with all results
    """
    ...
    if study_name is None:
        study_name = f"ldap_tuning_{dataset}_{int(time.time())}"
    ...
    log_file = f"logs/optuna_study_{dataset}.csv"
    ...
    def callback(study, trial):
        ...
        # Get trial results - include all hyperparameters and metrics
        result = {
    ...
    # Save summary
    summary_file = f"logs/optuna_summary_{dataset}.txt"
...
if __name__ == '__main__':
    ...
    print("\nTo visualize results, use Optuna's visualization tools:")
    print("  import optuna")
    print("  study = optuna.load_study(study_name='...', storage='sqlite:///optuna.db')")
